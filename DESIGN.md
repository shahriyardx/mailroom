...
3. **One accent.** It signals state: active view, unread, primary action,
...
```
┌────────────┬───────────────────┬──────────────────────────┐
│ sidebar    │ conversation list │ reading pane             │
│ 240px      │ 380-420px         │ fills                    │
│            │                   │                          │
│ wordmark   │ tabs: all/unread  │ sender chip, recipients  │
│ search ⌘K  │ rows: avatar,     │ subject, body            │
│ nav groups │ name, subject,    │ attachment cards         │
│ favourites │ 2-line snippet    │ reply composer           │
└────────────┴───────────────────┴──────────────────────────┘
```
...
1. Add Radix packages, build `kit/` alongside the existing `ui/`.
2. Move screens over one at a time: sign-in, shell, list, reading pane,
   composer, settings.
3. Delete `src/components/ui/` and the shadcn import from `globals.css` once
   nothing references them.
4. Retune tokens to the light, airy palette as the first step, since every
...
- Density: a conversation row stays two lines.
